...
public class ImageDownloader:ObservableObject{
    ...
    public init(url:String? = nil,urls:[String]? = nil,mode:String = "single",quality:JPEGQuality = .lowest,size:CGSize? = nil){
        self.mode = mode
    ...
    public func parseImage(data: Data,url safeURL:URL,bound:CGSize? = nil){
//        guard let safeData = UIImage(data: data)?.jpeg(self.quality), let safeImage = UIImage(data: safeData) else {return}
        guard let safeImage = UIImage(data: data) else {
            if self.loading{
                self.loading.toggle()
            }
            return
    ...
    public func downloadImg(url safeURL:URL,mode:String = "single",crop:Bool=false,bounds:CGSize? = nil){
    ...
    public func getImage(url:String,crop:Bool=false,bounds:CGSize? = nil){
        DispatchQueue.main.async {
            if !self.loading {self.loading = true}
        }
    ...
    public func getImages(urls:[String]){
