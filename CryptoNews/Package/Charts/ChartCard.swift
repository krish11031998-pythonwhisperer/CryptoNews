...
public struct ChartCard<T:View>: View {
    var header:String
    var size:CGSize
    var insideView:((CGFloat,CGFloat) -> T)
    var aR:ContentMode
    
    public init(header:String = "Header",size:CGSize  = .init(width: totalWidth * 0.5, height: totalHeight * 0.5),aR:ContentMode = .fill, @ViewBuilder insideView:@escaping ((CGFloat,CGFloat) -> T)){
        self.header = header
        self.size = size
